...
export const validateReservationForm = (
  ...
  if (facility?.status !== '운영중') errors.facilityId = '사용 가능한 시설만 등록할 수 있습니다.';
  if (!values.purpose.trim()) errors.purpose = '사용 목적 및 기타 정보를 입력하세요.';
  ...
  if (!errors.startAt && start.getTime() < now.getTime()) errors.startAt = '과거 시간은 사용일정으로 등록할 수 없습니다.';
  ...
  if (!errors.startAt && !errors.endAt && hasReservationConflict(values, reservations, ignoredReservationId)) {
    errors.overlap = '동일 시간대에 이미 등록된 사용일정이 있습니다.';
  }
